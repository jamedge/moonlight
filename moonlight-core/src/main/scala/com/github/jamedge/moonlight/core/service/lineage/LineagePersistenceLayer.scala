...
import com.github.jamedge.moonlight.core.model.neo4j.LineageQueries
  ...
  def getLineageGraph(rootIOElementName: String): Future[Graph[IOElement, LDiEdge]] = {
    neo4jDriver.readSession { implicit session =>
      session.transact[Graph[IOElement, LDiEdge]] { implicit tx =>
        for {
          rawEdges <- LineageQueries.constructGetLineageRawEdgesQuery(rootIOElementName).query[GraphBuilder.RawEdge].list(tx)
