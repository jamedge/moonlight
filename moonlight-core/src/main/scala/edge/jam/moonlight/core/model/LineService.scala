...
class LineService(
  ...
  def getLineageGraphDownstream(rootIOElementName: String, outputType: LineageGraphDownstreamOutputType): Future[String] = {
    for {
      graph <- getLineageGraph(rootIOElementName)
      resultJson <- Future(getLineageGraphDownstreamOutput(graph, rootIOElementName, outputType))
    } yield resultJson
  }

  def getLineageGraphDownstreamOutput(ioGraph: Graph[IOElement, LDiEdge], rootIOElementName: String, outputType: LineageGraphDownstreamOutputType): String = {
